...
void matrix_display2 ( double *tab,int  N, int M ) {
  int i,j;

  for (i=0; i<N; i++) {
    for (j=0; j<M; j++) {
      printf ("%8.2f ", *(tab+i*M+j) );fflush(stdout);
    }
    printf ("\n");fflush(stdout);
  }
...
void gauss ( double * tab, int N, int me, int nproc ) {
  int i,j,k;
  double pivot;
  double lignek[N];
  int ilocal=0;
  for ( k=0; k<N-1; k++ ){ /* mise a 0 de la col. k */  
    if(k%nproc==me)
    { 
     //matrix_display2 ( tab + k/nproc*N , 1, N );
      memcpy(lignek, tab + k/nproc*N, N * sizeof(double));

      int dest = pvm_gettid(GRPNAME, i % nproc);
      //printf("envoie de %d \n", me);fflush(stdout);
      pvm_initsend( PvmDataDefault );
      pvm_pkdouble(tab+ (int)k/nproc*N, N, 1);
      pvm_bcast(GRPNAME, me);
      ilocal++;
    }else{
      //printf("reciv %d \n", me);fflush(stdout);
      pvm_recv(-1, -1);
      pvm_upkdouble(lignek, N, 1);

    }
  
    if ( fabs(lignek[k]) <= 1.0e-11 ) {
      printf ("ATTENTION: pivot %d presque nul: %g\n", k, (lignek[k]) );
      exit (-1);
    }
    //indice local
    for ( i = ilocal; i<N/nproc; i++ ){ /* update lines (k+1) to (n-1) */
      pivot = - *(tab+i*N + k) / *(lignek+k);
      for ( j=k; j<N; j++ ){ /* update elts (k) - (N-1) of line i */
	      *(tab+j+i*N) = *(tab+j+i*N) + pivot * *(lignek+j);
      }
      /* *(tab+k+i*N) = 0.0; */
    }
  }
...
//printf("azeazeazeaze %d \n", me);fflush(stdout);
  if (me == 0)
  {
    ...
    for (int i = 0; i < N ; i++)
    {
      if(i%nproc == 0){
        for (int j = 0; j < N ; j++)
          fprintf(f, "%8.2f ", *(tab+i/nproc*N + j));
        fprintf(f, "\n");
      }else{
        //printf("reciv %d \n", me);fflush(stdout);
        pvm_recv(-1, i% nproc);
        pvm_upkdouble(ligne, N, 1);
        //printf("moi %d | from %d | val %f\n", me,i%nproc, ligne[0]);fflush(stdout);
        for (int j = 0; j < N ; j++)
